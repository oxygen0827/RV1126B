...
var (
	...
	rgaDstFds  [2]int32
	rgaAnnoBuf []byte   // mmap 的 640x480x3 BGR 输出（实时标注视频用）
	rgaAnnoFd  int32
	rgaMu      sync.Mutex
)
...
func rgaSetup() error {
	...
	// 640x480 BGR 标注输出 buffer
	afd, abuf, err := dmaAlloc(640 * 480 * 3)
	if err != nil {
...
// rgaConvertToBGR2: 当前帧 fd -> rgaAnnoBuf (640x480 BGR), 供方案A实时叠加
...
// rgaConvertToBGR 用 RGA 把源帧（NV12/YUYV dma-buf fd）转成 640x480 BGR 到 rgaAnnoBuf
func rgaConvertToBGR(srcFd int32, srcFmt int32, w, h, ws, hs, srcBytes int) bool {
	if !rgaOK || srcFd < 0 {
		return false
	}
	// dst: rgaAnnoBuf 固定 640x480 BGR; src: NV12 (w,h) with (ws,hs)
	srcInfo := rgaMakeInfo(srcFd, srcFmt, 0, 0, int32(w), int32(h), int32(ws), int32(hs), int32(srcBytes))
	dstInfo := rgaMakeInfo(rgaAnnoFd, rkFmtBGR888, 0, 0, 640, 480, 640, 480, 640*480*3)
	if r := rgaLibBlit(unsafe.Pointer(&srcInfo[0]), unsafe.Pointer(&dstInfo[0]), nil); r != 0 {
		return false
	}
	return true
