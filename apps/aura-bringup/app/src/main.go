...
func runPipeline(src frameSource, frames int, annoPath, rawAnnoPath string, brightness float32, vw, vh int) error {
	...
	go func() {
		...
		for j := range jobs {
			...
			sumDec += decMs
			sumPersons += len(keep)
			// ==== 健身应用（从 RV1106 移植）：触发轮询 + 会话窗口 + 录制 ====
			if *demoFlag {
				pollFitnessUploadTrigger()
				if n%20 == 0 && demoRecS.pollUploadState() {
					speakVerdictOnOK()
				}
				if session != nil {
					...
					if !allow {
						...
						sessionWasAllowed = true
						var sessionKeep []detection
						if len(keep) > 0 {
...
func main() {
	...
	// ==== 健身应用初始化（从 RV1106 移植）====
	if *sessionFileFlag != "" {
		session = newPoseSession(*sessionFileFlag, *sessionSecondsFlag)
		fmt.Println("fitness session window:", *sessionFileFlag, *sessionSecondsFlag, "s")
	}
	if *ttsApiKeyFlag != "" || *ttsAudioDirFlag != "" || *ttsCmdFlag != "" {
		var zt *zhipuTTS
		if *ttsApiKeyFlag != "" {
			zt = newZhipuTTS(*ttsApiKeyFlag, *ttsApiUrlFlag, *ttsModelFlag, *ttsVoiceFlag,
				*ttsFormatFlag, *ttsCacheDirFlag, 30*time.Second, *ttsInsecureFlag)
		}
		tts = newTTSPlayer(zt, *ttsCmdFlag, *ttsAudioDirFlag, *ttsCooldownFlag)
		defer tts.Close()
	}
